...
class TextEditorRenderer {
    ...
    constructor() {
        this.editor = document.getElementById('rich-editor') as HTMLDivElement;
    ...
    private setupContextMenu(): void {
        ...
        this.editor.addEventListener('contextmenu', (e: MouseEvent) => {
            e.preventDefault();
     ...
     * Convert full text to formatted HTML lines (for initial load)
     */
    private formatAllContent(text: string): string {
        ...
        return text.split('\n').map(line => {
            ...
            // Quote (> or > )
                ...
                if (line === '>') content = ''; // Empty line case
     ...
     */
    private updateFilePathDisplay(): void {
     ...
     */
    private async handleShowChangelog(): Promise<void> {
        try {
     ...
     */
    private displayHelpContent(data: any): void {
    ...
    private displayChangelogContent(data: any): void {
            ...
            data.releases.forEach((release: any) => {
                ...
                if (release.changes) {
                    ...
                    release.changes.forEach((change: any) => {
                        ...
                        if (catLower.includes('add')) badgeClass = 'badge-added';
                        else if (catLower.includes('fix')) badgeClass = 'badge-fixed';
                        else if (catLower.includes('change') || catLower.includes('modern')) badgeClass = 'badge-changed';
    ...
    private handleEditorInput(): void {
        ...
        if (sel && sel.anchorNode) {
            let block = sel.anchorNode as Node;
    ...
    private updateBlockContent(block: HTMLElement, newHTML: string, wasActive: boolean): void {
     ...
     */
    private saveAndRestoreSelection(block: HTMLElement, action: () => void): void {
        const sel = window.getSelection();
     ...
     */
    private findTextNodeAt(element: Node, offset: number): { node: Node, offset: number } | null {
    ...
    private handleEditorKeydown(e: KeyboardEvent): void {
        ...
        if (e.key === 'Enter' && !e.shiftKey) {
            ...
            if (sel && sel.rangeCount > 0) {
                ...
                // Manual Split:
                // 1. Get current block
                let currentBlock = range.endContainer as Node;
                while (currentBlock && currentBlock.parentNode !== this.editor) currentBlock = currentBlock.parentNode!;
                ...
                if (newSel && newSel.anchorNode) {
                    let newBlock = newSel.anchorNode as HTMLElement;
                    while (newBlock && newBlock.parentElement !== this.editor) newBlock = newBlock.parentElement!;
